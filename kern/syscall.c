...
static int sys_tx_pkt(char* buf,size_t nbytes)
{
	user_mem_assert(curenv,(void*)buf,nbytes,0);
	return tx_pkt(buf,nbytes);
}

static int sys_rx_pkt(char* buf,size_t max_bytes)
{
	user_mem_assert(curenv,(void*)buf,max_bytes,0);
	return rx_pkt(buf,max_bytes);
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) 
	{
		...
		case SYS_TX_PKT:
			return sys_tx_pkt((char*)a1,a2);
		case SYS_RX_PKT:
			return sys_rx_pkt((char*)a1,a2);
		default:
			return -E_INVAL;
	}
}
