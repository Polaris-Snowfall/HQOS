...
page_init(void)
{
	...
	//将Base_memory(除了MPENTRY)全部设置为free.(经验证basemem指低0xA0000的内存)
...
page_alloc(int alloc_flags)
{
	...
	if(page_free_list==NULL)
		return result;
	physaddr_t pa = (physaddr_t)page2kva(page_free_list);
	struct PageInfo * next_flist;

	next_flist = page_free_list->pp_link;
	if(alloc_flags&ALLOC_ZERO)
		memset((void*)pa,0,PGSIZE);
	page_free_list->pp_link = NULL;
	page_free_list = next_flist;
	return result;
...
page_free(struct PageInfo *pp)
{
	...
	{
		dbg("page_free:%d,%p",pp->pp_ref,pp->pp_link);
		panic("page_free fail,double free or maintain refered");
	}
...
{
	if(PDX(va)>=NPDENTRIES)
		panic("out of index in pgdir");
	...
	if(pgdir[PDX(va)]==0)
	{
		...
		else
		{
			...
			if(pg_info==NULL)
				...
				pg_info->pp_ref++;
			pte = (pte_t*)page2pa(pg_info);
			pgdir[PDX(va)] = (pde_t)pte|PTE_P|PTE_U;
		}
	}
	return KADDR((physaddr_t)((pte_t*)PTE_ADDR(pte)+PTX(va)));
}
...
//
// C0orner-case hint: Make sure to consider what happens when the same
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if(pte_store!=NULL)
		*pte_store = pte;
	if(pte!=NULL&&PGNUM(*pte) < npages&&(*pte&PTE_P))
	{
		return pa2page(*pte);
	}
	return NULL;
